...
function DashBoard() {
  ...
  const onTeamsDropdownChange = (value) => {
  ...
  const fetchChallengesGraphData = async () => {
    const response = await dashboardManager.getChallengesChart(challengeFilter);
    if (response.success) {
      setChallengesChartdata(response.data);
    } else {
      setToastMessages((prevMessages) => [
        ...prevMessages,
        {
  ...
  const fetchTeamsGraphData = async () => {
    const response = await dashboardManager.getTeamsChart(teamsFilter);
    if (response.success) {
      setTeamsChartdata(response.data);
    } else {
      setToastMessages((prevMessages) => [
        ...prevMessages,
        {
